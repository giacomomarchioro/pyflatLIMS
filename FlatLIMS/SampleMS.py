...
class SamplesDB:
    def __init__(self,path,name='samples'):
        self.path=path
        self.name=name
        self.header=['id','Status','sample_Label','Name','Project','Dataset','Description',
    ...
    def check_integrity(self):
        import numpy as np
        ids = np.genfromtxt(r'%s\LabDatabase\SamplesDB\%s.csv' 
        %(self.path,self.name),delimiter=',',names=True,usecols='id',
    ...
    def add_sample(self):
        ...
        for i in self.header[1:-2]: #avoid looping over id and link
            fields.append(raw_input('%s : ' %(i)) )

        
        with open(r'%s\LabDatabase\%s.csv' %(self.path,self.name), 'a') as f:
            writer = csv.writer(f)
...
class EquipmentDB:
    ...
    def check_integrity(self):
        import numpy as np
        ids = np.genfromtxt(r'%s\LabDatabase\EquipmentDB\%s.csv' 
        %(self.path,self.name),delimiter=',',names=True,usecols='id',dtype=np.int).astype(np.int) 
        if ids.shape == ():        
            return ids
    ...
    def load_DB(self):
        import numpy as np
        data = np.genfromtxt(r'%s\LabDatabase\EquipmentDB\%s.csv' %(self.path,self.name),delimiter=',',names=True)
        return data
    ...
    def takeimage(self,ID,name,camera_port=1):
        ...
        # Take the actual image we want to keep
        camera_capture = get_image()
        file =r'%s\LabDatabase\EquipmentDB\Equipment images\%s' %(self.path,newid)
        # A nice feature of the imwrite method is that it will automatically choose the
        # correct format based on the file extension you provide. Convenient!
        cv2.imwrite(file, camera_capture)
...
samplesdb = SamplesDB(r'C:\Users\OPdaTe\Desktop')

equipmentdb =  EquipmentDB(r'C:\Users\OPdaTe\Desktop')
...
alabDB =  labDB(r'C:\Users\OPdaTe\Desktop')